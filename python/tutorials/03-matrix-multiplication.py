...
def benchmark(M, N, K, provider):
    M, N, K = 3456, 4096, 8192
    a = transform_lhs(torch.randn((M, K) if args.config[1] == 'N' else (K, M), device='cuda', dtype=torch.bfloat16) + 2)
    b = transform_rhs(torch.randn((K, N) if args.config[3] == 'N' else (N, K), device='cuda', dtype=torch.bfloat16) + 2)
