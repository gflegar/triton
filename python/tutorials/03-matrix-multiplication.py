...
def matmul_kernel(
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        accumulator += tl.dot(a.to(tl.bfloat16), b)
...
torch.manual_seed(0)
a = torch.randint(-100, 100, (512, 512), device='cuda', dtype=torch.int8)
b = torch.randn((512, 512), device='cuda', dtype=torch.bfloat16).T
triton_output = matmul(a, b)
torch_output = torch.matmul(a.to(torch.bfloat16), b)
...
def benchmark(M, N, K, provider):
    a = torch.randint(-100, 100, (K, N), device='cuda', dtype=torch.int8)
    b = torch.randn((N, K), device='cuda', dtype=torch.bfloat16).T
    quantiles = [0.5, 0.2, 0.8]
    if provider == 'cublas':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: torch.matmul(a.to(torch.bfloat16), b), quantiles=quantiles, rep=2000)
    if provider == 'triton':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: matmul(a, b), quantiles=quantiles, rep=2000)
